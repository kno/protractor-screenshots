...
(function() {
  ...
  var capabilitiesPromise = browser.getCapabilities().then(function(mycapabilities) {
...
*/
    browserName = mycapabilities.get("browserName").toLowerCase();
    platform = mycapabilities.get("platform").toLowerCase();
    version = mycapabilities.get("version").toLowerCase();
    capabilityString = "" + platform + "-" + browserName + "-" + version;
    return {mycapabilities:mycapabilities,capabilityString:capabilityString}
    });
  capabilitiesPromise.then(function(data){
    capabilities = data.mycapabilities;
    capabilityString = data.capabilityString;
  })
  ...
  matchesCapabilities = function(other) {
    ...
    return _.every(other, function(value, key) {
      ...
      return capabilities.get(key) === value;
    });
  };
  ...
  matchScreenshot = function(spec, screenshotName, screenshot) {
    ...
    path = screenshotBase + "/" + slug(spec) + "/" + (slug(capabilityString));

    label = "" + screenshotName + " - " + screenshot.width + "x" + screenshot.height;
    filename = "" + (slug(screenshotName)) + "-" + screenshot.width + "x" + screenshot.height + ".png";
